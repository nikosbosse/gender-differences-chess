...
scatter_plots <- function(data, top_x = Inf, 
  ...
  p1 <- scatter(plot_data_mean) + 
    labs(y = "Mean rating women", x = "Mean rating men")
  ...
  p2 <- plot_data_median %>%
    scatter() + 
    labs(y = "Median rating women", x = "Median rating men")
  ...
  p3 <- plot_data_sd %>%
    scatter() + 
    labs(y = "SD rating women", x = "SD rating men") 
  # + 
...
p_mean <- df %>%
  ...
  geom_point(size = 0.8) + 
  labs(x = "\u0394 mean rating men - women", 
...
p_sd <- df %>%
  ggplot(aes(x = diff_df, y = n_rel_F)) + 
  geom_point(size = 0.8) + 
  labs(x = "\u0394 sd rating men - women", 
       y = "Percentage of women in a country") + 
  geom_smooth(method = "glm", 
              method.args=list(family="binomial"), 
              alpha = 0.1, colour = 'grey', size = 0.4) + 
...
age_men_women <- country_by_country %>%
  ...
  # theme_minimal() + 
  labs(y = "Mean age women", x = "Mean age men")
...
agetop10 <- country_by_country %>%
  dplyr::group_by(country, sex) %>%
  dplyr::arrange(-rating) %>%
  dplyr::mutate(Gender = ifelse(sex == "M", "M", "W")) %>%
  dplyr::slice_head(n = 10) %>%
  dplyr::mutate(age = 2022 - birthday) %>%
  dplyr::group_by(country, Gender) %>%
  summarise(age = mean(age), rating = mean(rating)) %>%
  ggplot(aes(y = rating, x = age, group = Gender, fill = Gender, color = Gender)) + 
...
# load expected
expected <- fread("results/results_top10_mean.csv") %>%
...
rating_vs_age_top10 <- country_by_country %>%
  ...
  inner_join(expected) %>%
  mutate(rating_diff = rating_diff - Expected) %>%
  ...
  labs(y = "Observed \u0394 (mean rating M - F) - expected \u0394", x = "\u0394 mean age men - women")
