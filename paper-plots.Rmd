...
chess <- fread("data/ratings20191231_downloaded20211001.csv") %>%
  mutate(age = year(as.Date("2019-01-01")) - birthday)
...
```{r}
# data from Gyuri (from https://github.com/weijima/gender-gap-chess/tree/main/data)
rating_data <- readRDS("data/rating_data.rds")
```
...
rating_data_w_juniors_no_inactive <- rating_data %>%
  filter(sex != "", 
         active) 
rating_data_w_juniors_w_inactive <- rating_data |>
  filter(sex != "", 
         age > 18) 
rating_data_no_juniors_no_inactive <- rating_data %>%
  filter(sex != "", 
         age > 18,
         active) 
rating_data_no_juniors_w_inactive <- rating_data %>%
  filter(sex != "", 
         age > 18)
...
figure_4 <- function(data, 
                     regression_richard, add_label = "LABEL") {
...
```{r}
rating_data <- rating_data_w_juniors_no_inactive

#data <- data_w_juniors_no_inactive
...
figure_3_new <- function(data, rating_data,
                         regression_richard, add_label = "LABEL") {
